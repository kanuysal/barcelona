...
export function MiniMap({
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      const height = width / aspectRatio; // use current active dimensions
    ...
    const draw = () => {
      ...
      for (let i = 0; i < totalItems; i++) {
        ...
        ctx.fillStyle = isSelected
          ? "#000000" // Use elite black/gold theme rather than standard yellow
          : "rgba(0, 0, 0, 0.3)";
        ctx.fill();
      }
      ...
      if (!isFocused) {
        ...
        const viewHeight = 2 * Math.tan(vFov / 2) * (config.zoomOut || 31);
        const viewWidth =
          viewHeight *
