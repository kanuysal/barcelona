...
window.gsap = E; window.ScrollTrigger = ct;
...
let io = class {
  ...
  async setPlanes(e = 0, t = 0) {
    const s = W ? "/assets/videos/haute.mp4" : "/assets/videos/haute.mp4";
  ...
  ia = class {
    ...
    async setObject() {
      const e = await new Promise((o, n) => {
        this.loader.load(
          window.__MV_IMAGE || (W ? "/images/about/mv-image-sp.webp" : "/images/about/mv-image.webp"),
          o,
          void 0,
          n,
        );
...
const La = () => {
  ...
  Va = (u, e) => {
    const s = document.querySelector(".c-loading");
    // If no legacy preloader in DOM (custom preloader is used), skip entirely
    if (!s) {
      window.loading = !0;
      return;
    }
    if (s && s.dataset.skipLayoutPreloader === "true") {
      window.loading = !0;
    ...
    const i = E.timeline({ paused: !0, repeat: -1 }),
      o = E.timeline({
        delay: 0.15,
        onComplete: () => {
          (window.loading = !0);
          ...
          if (r) r.classList.remove("is-animation");
          n.forEach((h) => h && h.classList && h.classList.remove("is-animation"));
          i.pause();
        },
      }),
    ...
    if (!a) {
        if (s) E.to(s, { opacity: 0, pointerEvents: "none", duration: 0.7, onComplete: () => s.setAttribute("aria-hidden", "true") });
        window.loading = !0;
        return;
    }
    const loaderInner = document.querySelector(".c-loading__load-inner");
    if (loaderInner) {
        i.to(loaderInner, {
          rotate: "-360deg",
          duration: 1,
